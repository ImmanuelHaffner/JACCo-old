...
int main(int argc, char **argv) {
  ...
  char *sp = child_stack + kStackSize;
  printf("Parent: %p\n", sp);
  pid_t clone_pid = clone(Child, sp, CLONE_FILES | CLONE_VM, ((void *)0), 0, 0, 0);
  int status;
  pid_t wait_result = waitpid(clone_pid, &status, 0x80000000);
  if (wait_result < 0) {
    perror("waitpid");
    return 1;
  }
  if (wait_result == clone_pid && ((((__extension__ (((union { __typeof(status) __in; int __i; }) { .__in = (status) }).__i))) & 0x7f) == 0)) {
